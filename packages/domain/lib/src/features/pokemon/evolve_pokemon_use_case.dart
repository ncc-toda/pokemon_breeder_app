...
class EvolvePokemonUseCase extends _$EvolvePokemonUseCase {
  ...
  Future<EvolutionResult> evolve(int partyPokemonId) async {
    try {
      debugPrint('EvolvePokemonUseCase.evolve: Starting evolution for partyPokemonId=$partyPokemonId');
      ...
      if (partyPokemon.breedingCounter < 10) {
        debugPrint('EvolvePokemonUseCase.evolve: Breeding counter insufficient (${partyPokemon.breedingCounter})');
        return EvolutionResult.failure('育成カウンターが不足しています。（現在: ${partyPokemon.breedingCounter}/10）');
      }

      // 進化先の確認
      final evolutionTargetId = EvolutionData.getEvolutionTarget(partyPokemon.pokemonId);
      if (evolutionTargetId == null) {
        debugPrint('EvolvePokemonUseCase.evolve: No evolution target for pokemonId=${partyPokemon.pokemonId}');
        return EvolutionResult.failure('このポケモンは進化できません。');
      }

      debugPrint('EvolvePokemonUseCase.evolve: Evolution ${partyPokemon.pokemonId} -> $evolutionTargetId');
      ...
      debugPrint('EvolvePokemonUseCase.evolve: Evolution completed successfully');
  ...
  Future<EvolutionResult> devolve(int partyPokemonId) async {
    try {
      debugPrint('EvolvePokemonUseCase.devolve: Starting devolution for partyPokemonId=$partyPokemonId');
      ...
      // 退化先の確認
      final devolutionTargetId = EvolutionData.getDevolutionTarget(partyPokemon.pokemonId);
      if (devolutionTargetId == null) {
        debugPrint('EvolvePokemonUseCase.devolve: No devolution target for pokemonId=${partyPokemon.pokemonId}');
        return EvolutionResult.failure('このポケモンは退化できません。');
      }

      debugPrint('EvolvePokemonUseCase.devolve: Devolution ${partyPokemon.pokemonId} -> $devolutionTargetId');
      ...
      debugPrint('EvolvePokemonUseCase.devolve: Devolution completed successfully');
